...
This attack can be applied in different ways also, it only requires:
 1. The victim contract that is called during message relay calls another malicious contract.
 2. The victim contract requires a callback from the malicious contract.
...
Please note that, this attack surface was not possible in the first round of contest, because there was a reentrancy guard on `L1CrossDomainMessenger.relayMessage`. But, now that the classical reentrancy guard is removed, the novel reentrancy is vulnerable to this kind of attack.
...
## Recommendation
Maybe one solution is to map the `xDomainMsgSender` based on the hash of the transaction:
```solidity
